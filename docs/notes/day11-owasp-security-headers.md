...
## Практика на занятии (сделаем когда дойдём до Day 11)

> Задания по программе курса. Теория ниже — заранее. **На занятии** проходим чеклист и сдаём то, что требует teacher.

### Уже сделано досрочно

- [x] Security headers в nginx (`/etc/nginx/sites-available/andrii.mercantec.tech`)
- [x] `curl -I https://andrii.mercantec.tech/` — проверить CSP, HSTS, X-Frame

### Сделать на занятии / aflevering

**Мини-упражнение (15–20 мин):** выбрать **2 пункта** OWASP Top 10 и для каждого написать:
1. Как обычно возникает
2. Последствие
3. Одна mitigation для **MercantecApi**

**Security review (arbejdsopgave):** наша app `MercantecApi` / `andrii.mercantec.tech`:
1. Найти минимум **3 риска** (привязать к OWASP: A01, A03, A05…)
2. Для каждого: описание · likelihood/impact · **конкретная** mitigation
3. **Bonus:** внедрить ≥1 mitigation и описать (headers уже считаются)
4. **Рефлексия (1 абзац):** что удивило в headers / access control?

**Формат сдачи:** короткий отчёт 1–2 страницы (Teams / по инструкции teacher).
...
0. **Практика на занятии** — чеклист (сделаем когда дойдём до дня)
...
> ⬜ Day 11 — частично (headers ✅ · review ⬜)

> **Практику из чеклиста ниже** (кроме уже сделанного) выполним **когда дойдём до Day 11** на курсе.
